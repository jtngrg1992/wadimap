...
<div class="login-dialog" hidden>
	<form action="login.php?uae" method="post">
...
<script>
	wadi="NA";
	source="<?php if (isset($_SESSION['source']))echo $_SESSION['source'];else echo 'wadi_uae';?>";
	count=1;
	souq="NA"
	id=1;
	jumpid=0;
	...
	$(document).ready(function () {	
        ...
        if(user=="NA")
			...
			$('.welcome').html("<div class='dropdown'><p>Options</p>" +
					"<ul class='dropdown-content'>" +
					"<li><a href='#' onclick='switchView()'>Switch to uploads</a></li>" +
                    "<li><a href='#'>Browse through mappings</a></li>"+
					"<li><a href='#' onclick='logout()'>Logout</a></li></ul></div>" )
...
{	//making ajax to retrieve wadi product
		$.when($.ajax({
			type: 'POST',
			url: 'get_wadi.php?uae',
			async: false,
			data: {item: id,source:source},
